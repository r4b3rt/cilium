...
This upgrade guide is intended for Cilium running on Kubernetes.  It is
assuming that Cilium has been deployed using one of the standard installation
guides. If you are unsure whether this guide applies for your environment or if
you have questions, feel free to ping us on the `Slack channel`.
...
Optional: If you want to rebase your `ConfigMap` to the latest template, refer
to the section :ref:`upgrade_configmap` for instructions on how to rebase a
`ConfigMap` to the latest template while preserving your configuration
parameters.
